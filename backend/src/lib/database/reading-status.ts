 ...
 */
export async function getPendingReadings(limit: number = 10) {
 ...
 */
export async function markReadingAsProcessing(readingId: string) {
  try {
 ...
 */
export async function markReadingAsCompleted(readingId: string, readingData: any) {
  try {
    return await prisma.reading.update({
      where: { id: readingId },
      data: {
 ...
 */
export async function markReadingAsFailed(readingId: string, errorMessage: string) {
  try {
    return await prisma.reading.update({
      where: { id: readingId },
      data: {
