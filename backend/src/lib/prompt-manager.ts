...
export class PromptManager {
  ...
  public async getPerformanceAnalytics(
    ...
    const versionGroups = template.testResults.reduce((acc: any, result: any) => {
    ...
    const versions = Object.entries(versionGroups)
      .map(([version, results]: [string, any]) => {
        const successful = (results as any[]).filter(
          (r: any) => r.resultData && !r.resultData.error
        ).length;
        const avgTime =
          (results as any[]).reduce((sum: any, r: any) => sum + (r.executionTimeMs || 0), 0) /
          (results as any[]).length;
        const avgTokens =
          (results as any[]).reduce((sum: any, r: any) => sum + (r.tokenUsage || 0), 0) /
          (results as any[]).length;
        ...
        return {
          ...
          metrics: {
            ...
            totalTests: (results as any[]).length,
          },
