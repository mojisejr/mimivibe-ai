...
exports.parseAndValidateAIResponse = parseAndValidateAIResponse;
...
function tryFixTruncatedJson(jsonString) {
    try {
        ...
        if (openBraces > closeBraces || openBrackets > closeBrackets) {
            ...
            const lastCompleteIndex = Math.max(fixed.lastIndexOf('"}'), fixed.lastIndexOf('"]'), fixed.lastIndexOf('}'), fixed.lastIndexOf(']'));
...
function parseAIResponse(response) {
    try {
        ...
        const result = { success: false, rawResponse: cleanResponse };
 ...
 */
function parseAndValidateAIResponse(response, requiredFields) {
    const parsed = parseAIResponse(response);
    if (!parsed.success) {
        return parsed;
    }
    const validated = validateJsonStructure(parsed.data, requiredFields);
 ...
 */
function logParsingError(node, response, error) {
