...
export function ChatContainer({ className = '' }: ChatContainerProps) {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                if (data.event === 'reading') {
                  ...
                  setMessages(prev => [...prev, cardsMessage, readingMessage])
                }
                ...
                if (data.event === 'error') {
                  ...
                  const errorMessage: Message = {
                    id: `error-${Date.now()}`,
                    type: 'assistant',
                    content: 'ขออภัยค่ะ เกิดข้อผิดพลาดในการดูดวง กรุณาลองใหม่อีกครั้งค่ะ',
                    timestamp: new Date()
                  }
      ...
      const errorMessage: Message = {
        id: `error-${Date.now()}`,
        type: 'assistant',
        content: 'ขออภัยค่ะ เกิดข้อผิดพลาดในการเชื่อมต่อ กรุณาตรวจสอบการเชื่อมต่ออินเทอร์เน็ตและลองใหม่อีกครั้งค่ะ',
        timestamp: new Date()
      }
  ...
  return (
    <div className={`flex flex-col h-full bg-gradient-to-b from-base-100 to-base-200 ${className}`}>
      <ChatHeader />
      ...
      <div className="border-t border-base-300 bg-base-100/80 backdrop-blur-sm">
        <ChatInput 
          ...
          placeholder={isProcessing ? "กำลังดูดวง..." : "พิมพ์คำถามของคุณที่นี่..."}
        />
      </div>
    </div>
  )
